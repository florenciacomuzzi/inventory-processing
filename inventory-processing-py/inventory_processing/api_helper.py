    ...
    def _get_csrf_token(self):
        ...
        try:
            ...
            if not self.csrf_token:
                auth_token_match = re.search(r'<input[^>]+name="authenticity_token"[^>]+value="([^"]+)"', response.text)
                if auth_token_match:
                    self.csrf_token = auth_token_match.group(1)
                    print(f"Found CSRF token in input field: {self.csrf_token}")
            
            if not self.csrf_token:
                print("Warning: Could not find CSRF token in meta tag, cookies, or input field")
    ...
    def post(self, endpoint: str, data, chunk_size: int = 50):
        ...
        for item in data:
            ...
            if cleaned_item.get('internal_id') and cleaned_item['internal_id'].startswith('biz_id_'):
                cleaned_item['internal_id'] = cleaned_item['internal_id'].replace('biz_id_', '')
        ...
        print(f"Splitting {len(cleaned_data)} items into {len(chunks)} chunks of {chunk_size} items each")
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            try:
                ...
                print(f"Chunk {i} response text (first 500 chars): {response_text[:500]}")
                ...
                if response_text.strip().startswith('<!DOCTYPE html>'):
                    ...
                    error_match = re.search(r'<div class="error">(.*?)</div>', response_text, re.DOTALL)
                    if error_match:
                        error_message = error_match.group(1).strip()
                        print(f"Error message from HTML: {error_message}")
                    raise ValueError("Server returned HTML instead of JSON response")
